...
export default function ProductPage() {
  ...
  useEffect(() => {
    ...
    async function fetchMessages() {
      try {
        ...
        if (!token) return
        const response = await fetch(`http://localhost:4000/api/chats/messages/${item.seller_id}`, {
  ...
  async function handleAddToCart() {
    ...
    try {
      const response = await fetch('http://localhost:4000/api/cart/' + activeUserId, {
        method: 'POST',
  ...
  async function submitReview() {
    ...
    const response = await fetch('http://localhost:4000/api/reviews/product/' + routeProductId, {
      method: 'POST',
